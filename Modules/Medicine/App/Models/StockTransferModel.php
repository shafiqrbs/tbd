...
class StockTransferModel extends Model
{
    ...
    public static function getRecordsForCentralBK($request, $domain)
    {
        // Pagination setup
        $page = isset($request['page']) && $request['page'] > 0 ? ($request['page'] - 1) : 0;
        $perPage = !empty($request['offset']) ? (int)$request['offset'] : 50;
        ...
        // 🔹 Map stockTransferItems with filtered purchase items
        $entities = $entities->map(function ($transfer) {
            $transfer->stockTransferItems = $transfer->stockTransferItems->map(function ($item) {
    ...
    public static function getDetails_BK($id)
    {
    ...
    public static function getDetails($id)
    {
        ...
        // 🔹 Map stockTransferItems with filtered purchase items
        if ($entity) {
            $entity->stockTransferItems = $entity->stockTransferItems->map(function ($item) {
                $item->purchaseItems = $item->purchaseItems->map(function ($p) {
                    $salesQty = $p->sales_quantity ?? 0;
                    return [
                        'id'                => $p->id,
    ...
    public static function insertStockTransferItems($stockTransfer, array $items, int $configId): bool
    {
        ...
        foreach ($items as $record) {
            $insertData[] = [
                ...
                'request_quantity' => $record['quantity'],
