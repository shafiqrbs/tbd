...
class StockItemRepository extends EntityRepository
{
    ...
    public function getItemOpeningSummary(Item $item , $field = "")
    {
        ...
        $qb->join("e.item",'i');
        $qb->where("i.id = :id")->setParameter('id',$item->getId());
    ...
    public function processStockQuantity($item , $fieldName = ''){
        ...
        $em = $this->_em;
         ...
         $openting = $this->getItemOpeningSummary($item->getItem(),$fieldName);
        ...
        if($fieldName == 'purchase'){
            ...
            if($openting){
                $opening = $this->findOneBy(array('config'=> $entity->getConfig(),'item'=> $entity->getItem() ,'code' => $openting));
          ...
          $entity->setRemark('Debit-note');
            ...
            $entity->setItem($item->getItem());
            $entity->setPrice($entity->getItem()->getSalesPrice());
            ...
            $entity->setPrice($item->getItem()->getSalesPrice());
            $entity->setMode('production-stock');
            ...
            $entity->setItem($item->getItem());
            $entity->setPrice($item->getItem()->getPurchasePrice());
            $entity->setDamage($item);
            $entity->setSubTotal($item->getQuantity() * $item->getItem()->getPurchasePrice());
            $entity->setTotal($item->getQuantity() * $item->getItem()->getPurchasePrice());
            ...
            $item = $em->getRepository('TerminalbdInventoryBundle:PurchaseItem')->find($id);
        ...
        $entity->setConfig($item->getItem()->getConfig());
    ...
    {
        $em = $this->_em;
        $closing = $this->getItemOpeningSummary($entity->getItem(),$fieldName);
    ...
    {
        $em = $this->_em;
        if($item->getItem()->getMasterItem()->getSlug() == "finish-goods"){
            $openting = $this->getItemOpeningSummary($item->getItem(),'purchase-sales');
        }else{
            $openting = $this->getItemOpeningSummary($item->getItem(),'purchase');
        }
        $entity = new StockItem();
        $entity->setConfig($item->getItem()->getConfig());
        if($item->getCustomsDuty() > 0){
    ...
    {
        $em = $this->_em;
        $openting = $this->getItemOpeningSummary($item->getItem(),'purchase-return');
        $entity = new StockItem();
        $exist = $this->findOneBy(array('purchaseReturnItem' => $item, 'mode' => 'purchase-return'));
        if ($exist) {
            $entity = $exist;
        ...
        $entity->setRemark('Debit-note');
        if($openting){
            $opening = $this->findOneBy(array('config'=> $item->getItem()->getConfig(),'code' => $openting));
            $entity->setOpeningQuantity(floatval($opening->getClosingQuantity()));
            $entity->setOpeningBalance(floatval($opening->getClosingBalance()));
        }else{
            $entity->setOpeningQuantity(0);
    ...
    public function rawMaterialConsumtion(ProductionExpense $expense,PurchaseItem $purchaseItem,float $quantity){
        ...
        $openting = $this->getItemOpeningSummary($expense->getItem(),"production-expense");
    ...
    public function inHouseProductionStock(ProductionBatchItem $item)
    {
        ...
        $openting = $this->getItemOpeningSummary($item->getItem(),"production-stock");
    ...
    public function contructualDirectProductionExpenBatchItemRetun(ProductionBatchItem $item, ProductionExpense $expense)
    {
        ...
        $returnProductionBatchItem = $this->findBy(array('productionBatchItem' => $item,'item' => $expense->getItem()));
        /* @var $stockItem StockItem */

        foreach ($returnProductionBatchItem as $stockItem):

            $quantity = $expense->getReturnQuantity();
            $openting = $this->getItemOpeningSummary($expense->getItem(),'production-expense-return');


            $entity = new StockItem();
            $exist = $this->findOneBy(array('item' => $expense->getItem(),'productionBatchItem' => $item,'mode' => 'production-expense-return'));
            if($exist){ $entity = $exist; }
            $entity->setConfig($expense->getItem()->getConfig());
            $entity->setQuantity($quantity);
            $entity->setProductionExpenseReturnQuantity($quantity);
            $entity->setItem($expense->getItem());
    ...
    public function contructualProductionExpenBatchItemRetun(ProductionBatchItem $item, ProductionExpense $expense)
    {
        ...
        foreach ($returnProductionBatchItem as $stockItem):
            ...
            if($stockItem->getProductionExpenseQuantity() >= $expense->getReturnQuantity() and $reminQnt > 0) {
                ...
                $quantity = $expense->getReturnQuantity();
                $openting = $this->getItemOpeningSummary($expense->getItem());

                $entity = new StockItem();
                $exist = $this->findOneBy(array('purchaseItem' => $stockItem->getPurchaseItem() ,'item' => $expense->getItem(),'productionBatchItem' => $item,'mode' => 'production-expense-return'));
                if($exist){ $entity = $exist; }
                $entity->setQuantity($quantity);
                $entity->setProductionExpenseReturnQuantity($quantity);
                $entity->setItem($expense->getItem());
                ...
                $entity->setConfig($entity->getItem()->getConfig());
                $em->persist($entity);
                $em->flush();
    ...
    public function getSalesInsertQnt(Sales $entity){
        ...
        if($entity->getSalesItems()){
            ...
            foreach($entity->getsalesItems() as $item ){
                if($item->getItem()->getMasterItem()->getProductGroup()->getSlug() == "finish-goods"){
                ...
                }elseif ("production-item" == $item->getItem()->getMasterItem()->getProductGroup()->getSlug()){
    ...
    public function processSalesItemProductionStock(SalesItem $item)
    {
        ...
        $openting = $this->getItemOpeningSummary($item->getItem(),"sales");
        $entity = new StockItem();
        $entity->setConfig($item->getItem()->getConfig());
        $entity->setQuantity($item->getTotalQuantity());
        $entity->setSalesQuantity($item->getTotalQuantity());
    ...
    public function processSalesReturnProductionItemStock(SalesReturnItem $item)
    {
        ...
        $openting = $this->getItemOpeningSummary($item->getItem(),"sales-return");
        $entity = new StockItem();
    ...
    public function finishGoodsConsumtion(SalesItem $salesItem,PurchaseItem $purchaseItem,$quantity){
        ...
        $openting = $this->getItemOpeningSummary($salesItem->getItem());

        /* @var  $entity StockItem */

        $entity = new StockItem();
        $exist = $this->findOneBy(array('salesItem' => $salesItem ,'item' => $salesItem->getItem() ,'purchaseItem' => $purchaseItem ,'mode' => 'sales'));
        if($exist){ $entity = $exist; }
        $entity->setQuantity('-'.$quantity);
        $entity->setSalesQuantity($quantity);
        $entity->setItem($salesItem->getItem());
    ...
    public function processSalesReturnStockQuantity(SalesReturnItem $salesReturnItem, $quantity){
        ...
        foreach ($salesReturnItem->getSalesItem()->getStockItems() as $stockItem ){
            ...
            if($stockItem->getSalesQuantity() >= $reminQnt and $reminQnt > 0){
                ...
                $openting = $this->getItemOpeningSummary($salesReturnItem->getItem());

                $entity = new StockItem();

                $exist = $this->findOneBy(array('salesReturnItem' => $salesReturnItem ,'item' => $stockItem->getItem() ,'salesItem' => $stockItem->getSalesItem(),'productionBatchItem' => $stockItem->getProductionBatchItem() , 'mode' => 'sales-return'));
                if($exist){ $entity = $exist; }
                ...
                $entity->setSubTotal($entity->getQuantity() * $entity->getPrice());
                $entity->setTotal($entity->getSubTotal());
                $entity->setItem($stockItem->getItem());
                ...
                $entity->setConfig($salesReturnItem->getItem()->getConfig());
                $em->persist($entity);
                $em->flush();
                $em->getRepository('TerminalbdProductionBundle:ProductionBatchItem')->updateBatchSalesReturnItem($stockItem->getProductionBatchItem());
                $this->itemUpdateClosingBalance($entity);
