...
class ProductRepository extends EntityRepository
{
    ...
    public function insertInstantItem(Inventory $inventory, MasterItem $masterItem , $data)
    {
        $em =  $this->_em;
        $name = $data['name'];
        $existMasterItem = $this->findOneBy(array('config' => $inventory,'masterItem' => $masterItem,'name'=> $name));
        if($existMasterItem){
            return $existMasterItem;
        }else{
            $post = new Item();
            $post->setConfig($inventory);
            $post->setMasterItem($masterItem);
            $post->setName($name);
            $post->setUnit($masterItem->getUnit());
            $em->persist($post);
            $em->flush();
        }
    ...
    public function updateTaxValue(Item $entity){
        ...
        if($tax){
            ...
            $entity->setRegulatoryDuty($tax->getCustomsDuty());
            $entity->setValueAddedTax($tax->getCustomsDuty());
            $entity->setAdvanceTradeVat($tax->getCustomsDuty());
            $entity->setSupplementaryDuty($tax->getCustomsDuty());
            $entity->setAdvanceTax($tax->getCustomsDuty());
            $entity->setAdvanceIncomeTax($tax->getCustomsDuty());
        }
    ...
    public function findWithSearch( $config, $parameter , $data )
    {
