...
class StockItemHistoryModel extends Model
{
    ...
    public static function openingStockQuantity($purchaseItem, $process = 'opening')
    {
        ...
        $data = [
            ...
            'quantity' => $process==='opening' ? 0 : $purchaseItem->quantity ?? 0,
            'opening_quantity' => $process==='opening' ? $purchaseItem->quantity ?? 0 : 0,
            'closing_quantity' => $process==='opening' ? $purchaseItem->quantity ?? 0 : 0,
            'closing_balance' => $process==='opening' ? $purchaseItem->sub_total ?? 0 : 0,
