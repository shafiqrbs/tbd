...
class PosController extends Controller
{
    ...
    public function checkInvoiceMode()
    {
        ...
        if ($config && $config->pos_invoice_mode) {
            // for imvoice mode is table
            if ($config->pos_invoice_mode->slug === 'table'){
//                dump('table');
                $tableData = ParticularModel::getEntityDropdown($this->domain['config_id'],'table');
                dump($tableData);
            }elseif ($config->pos_invoice_mode->slug === 'customer'){
                dump('customer');
            }elseif ($config->pos_invoice_mode->slug === 'user'){
                dump('user');
            }
//            dump($config->pos_invoice_mode);
        }
        /*$dataModel = ConfigModel::with('pos_invoice_mode')->where('id',$this->domain['config_id'])->first();
        dump($dataModel);*/

//        $getConfigData = ConfigModel::with('pos_invoice_mode')->find($this->domain['inv_config']);
        /*$findInvoiceModeId = ConfigModel::select('pos_invoice_mode_id')->find($this->domain['config_id']);
        if (!empty($findInvoiceModeId)) {
            $findInvoiceMode = SettingModel::find($findInvoiceModeId->pos_invoice_mode_id);
            dump($findInvoiceMode);
        }*/

//        dump($findInvoiceModeId->pos_invoice_mode_id);

//        dump($this->domain['inv_config']);
    }
