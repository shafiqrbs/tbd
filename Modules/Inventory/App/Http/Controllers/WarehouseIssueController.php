...
use Modules\Inventory\App\Http\Requests\WarehouseIssueRequest;
...
class WarehouseIssueController extends Controller
{
     ...
     */
    public function store(WarehouseIssueRequest $request)
    {
        DB::beginTransaction();
        ...
        try {
            ...
            if (!empty($input['items'])) {
                ...
                foreach ($input['items'] as $val) {
                    $stockItem = StockItemModel::find($val['stock_item_id']);
                    if (!$stockItem) {
                        throw new \Exception("Stock item not found");
                    }

                    $findProduct = ProductModel::find($stockItem->product_id);
                    if (!$findProduct) {
                        throw new \Exception("Product not found");
                    }

//                    dump($stockItem,$findProduct);

                    $total += $val['sub_total'];
                    $itemsToInsert[] = [
                        'requisition_id' => $requisition->id,
                        'customer_stock_item_id' => $val['stock_item_id'],
                        ...
                        'warehouse_id' => $val['product_warehouse_id'] ?? null,
                        'unit_id' => $findProduct->unit_id,
                        'unit_name' => $stockItem->uom,
                        'created_at' => now(),
                    ];
                }
